...
##            sg.applyDoubleAlgorithm(metricName, resultProperty, dataSet)
    ...
    for t in range(time_0, time_final, timestep):
        sg = graph.getSubGraph(str(t)) # I will need the non-deparallelized subgraph later
    ...
    print ('DP subgraphs saved. Now computing node-specific network properties...')
...
##    # compute node-specific network properties
    ...
    tlp.saveGraph(graph, dirPath+"/To_Guy/ERgraph_w_dp_subgraphs_10.tlp")


        # the properties that follow store activity data as node properties
