...
##    tlp.saveGraph(graph, dirPath+"/ERgraph_w_fe_subgraphs_57.tlp")
    ...
    for topic in topics:
        ...
        activityMetricNames.append('sn_EPosts in ' + topic)
        activityMetricNames.append('tn_NPosts in ' + topic)
        activityMetricNames.append('tn_EPosts in ' + topic)

        activityMetricNames.append('sn_NComms written in ' + topic)
        activityMetricNames.append('sn_EComms written in ' + topic)
        activityMetricNames.append('tn_NComms written in ' + topic)
        activityMetricNames.append('tn_EComms written in ' + topic)

        activityMetricNames.append('sn_NTeam comms received in ' + topic)
        activityMetricNames.append('sn_ETeam comms received in ' + topic)
        activityMetricNames.append('tn_NTeam comms received in ' + topic)
        activityMetricNames.append('tn_ETeam comms received in ' + topic)

        activityMetricNames.append('sn_NComms received in ' + topic)
        activityMetricNames.append('sn_EComms received in ' + topic)
        activityMetricNames.append('tn_NComms received in ' + topic)
        activityMetricNames.append('tn_EComms received in ' + topic)

    fieldNames = ['sn_K-Cores', 'tn_graphDensity', 'tn_currentCluster', 'tn_modularityCount', 'sn_modularityCount', 'sn_team', 'sn_Cluster', 'sn_Page Rank',\
    								   'tn_Page Rank', 'tn_averageClusteringCoefficient', 'tn_team', 'sn_graphDensity', 'source', 'tn_K-Cores', 'tn_betweennessCentralityCount',\
    								   'sn_user_id', 'sn_timestamp', 'sn_betweennessCentralityCount', 'sn_averageClusteringCoefficient', 'sn_currentCluster', 'tn_timestamp',\
    								    'target', 'tn_user_id', 'tn_Cluster', 'edge','sn_Indegree', 'tn_Indegree', 'sn_Outdegree', 'tn_Outdegree'] + activityMetricNames
...
##    print ('***')
    ...
    # for t in range(time_0, time_final, timestep):
    for t in range(time_0, time_0+1*timestep, timestep):
        ...
        for sourceNode in fesg.getNodes():
            ...
            for topic in topics:
                ...
                sourceNodeMetrics['EPosts in ' + topic] = 0
                sourceNodeMetrics ['NComms written in ' + topic] = 0
                sourceNodeMetrics ['EComms written in ' + topic] = 0 
                sourceNodeMetrics ['NTeam comms received in ' + topic] = 0 
                sourceNodeMetrics ['ETeam comms received in ' + topic] = 0
                sourceNodeMetrics ['NComms received in ' + topic] = 0
                sourceNodeMetrics ['EComms received in ' + topic] = 0 
                   ...
                   (time.mktime(time.strptime(postslist[x]['date'], "%d %b %Y - %H:%M")) >= t - timestep):
                    for topic in topics:
                        if postslist[x]['topic'] == topic:
                            ...
                            sourceNodeMetrics['EPosts in ' + topic] += int(postslist[x]['effort'])
            ...
            for e in sg.getInEdges(sourceNode):
                ...
                if dateProperty[e] >= (t - timestep):
                    ...
                    if is_team[e]:
                        ...
                        sourceNodeMetrics['ETeam comms received in ' + eTopic] += effort
                    else:
                        sourceNodeMetrics['NComms received in ' + eTopic] += 1
                        sourceNodeMetrics['EComms received in ' + eTopic] += effort
            ...
            for e in sg.getOutEdges(sourceNode):
                ...
                if dateProperty[e] > (int(t) - timestep) and dateProperty[e] <= int(t): # not sure about this
                    ...
                    sourceNodeMetrics['EComms written in ' + eTopic] += effort
	        ...
	        targetNodeMetrics = getNodeMetrics(targetNode, dpsg, nodeMetricNames) # grab node metrics via a function
                ...
                for e in sg.getInEdges(targetNode):
                    ...
                    if dateProperty[e] >= (t - timestep):
                        ...
                        if is_team[e]:
                            ...
                            targetNodeMetrics['ETeam comms received in ' + eTopic] += effort
                        else:
                            targetNodeMetrics['NComms received in ' + eTopic] += 1
                            targetNodeMetrics['EComms received in ' + eTopic] += effort
                ...
                for e in sg.getOutEdges(targetNode):
                    ...
                    if dateProperty[e] > (int(t) - timestep) and dateProperty[e] <= int(t): 
                        ...
                        targetNodeMetrics['EComms written in ' + eTopic] += effort
